...
# Abstract
By analyzing responses provided by the Canadian General Social Survey, we modeled an individuals mental health self-rating given demographic and familial information. We that our model makes nontrivial predictions and that there is a strong relationship between an individual's self-rating of their physical health and an individual's self-rating of their mental health (but did not find that our other explanatory variables were statistically significant). These findings indicate that mental health rating is very unpredictable and it is hard to accurately predict an individuals mental health from our selected explanatory variables. However, our results do show that working towards improving an individual's physical health could improve their own mental health.


# Introduction
Mental illness affects the well-being and the quality of life for people from all walks of life. Furthermore, mental illness is far more prevalent that you might think. According to the Centre for Addiction and Mental Health, every other Canadian will have had a mental illness by the age of 40. Our goal when exploring the responses from the 2017 General Social Survey with regards to mental health was to build a model that is able to accurately predict self-rated mental health and to identify the demographic and family traits that are strongest at accurately predicting mental health. After choosing 5 explanatory variables (age, sex, marital status, has children, and self-rated physical health) based on the demographics mentioned in mental health statistics, we fit a multinomial logistic regression model on those variables in an attempt to predict self-rated mental health. 

Our analysis included finding graphical correlations between the explanatory variables and self-rated mental, analyzing the coefficients of our converged model, conducting hypothesis testing on the statistical significance of the coefficients, and evaluating the accuracy of our model using cross validation. Overall, we found that our model is able to make non-trivial predictions on an individual's mental health given information about their age, sex, marital status, self-rated physical health, and if they have children. Furthermore, we found that of our five explanatory variables, the strongest predictor of self-rated mental health was self-rated physical health. We found that in addition to a strong graphical correlation, the coefficients for self-rated physical health were statistically significant at a significance level of 0.05. Unfortunately, the rest of variables did not perform as well. Although marital status had nonzero coefficients, our statistical test revealed that they were not statistically significant. The remaining three variables had coefficients that were close to zero, indicating the variables would not be helpful in differentiating an "Excellent" mental health self-rating from a "Poor" mental health self-rating.

The following section of this paper discusses how the dataset we used was collected and provides a summary of the dataset features and visualization of the distribution of variables of interest. Afterwards, we discussed our process when selecting and running our model and we presented the mathematical representation of our model. Lastly, our paper presents our results (as mentioned in the previous paragraph), discusses our results, and concludes with identifying weaknesses and next steps. 
...
Due to problems with the high nonresponse rate for questions relating to income, we attempted to use age and gender as indicators of income (Elkins, 2019) in an attempt to circumvent those problems. A study by McBride (2010) showed that peoples view on happiness is related to income, to a certain degree, and as such we expected to see people increasing in age to also increase the likelihood of reporting their mental health is very good or excellent, as well as males reporting a mental health in comparison to females (based on the general notion of the wage gap between the sexes). However, as we found, from our analysis of the coefficients, these two explanatory variables do not significantly change the outcome of the response variable thus putting an end to this train of analysis.
...
In summary, although we have found that our model is able to make non-trivial predictions about mental health self-ratings given demographic and family traits for an individual, we also determined that not all of our chosen explanatory variables are equally important in making that prediction. Based on graphical correlations, model coefficients, and statistical tests, we found that by and large self-rated physical health was the best explanatory variable with regards to self-rated mental health. Although there is a graphical correlation and non-zero model coefficients for marriage status and mental health, our statistical test suggests that this relationship is not statistically significant. Many values that are societally deemed to be strong predictors of mental health such as age, sex or having children did not seem to affect our models prediction on mental health. Overall, these conclusions potentially contribute to finding what can cause an individual to view their mental health in a certain way. This analysis shows that hitting certain benchmarks (earning a certain income, get married, reaching a certain age, etc.) does not seem to impact an individual's mental health like we assumed. 

# Weaknesses and Future Work

A major weakness of our methodology is our choice of a variable that is hard to predict, prone to bias, and relative to the respondents experience towards mental health. Tied into this is the fact that there are likely other variables we haven't considered that better predict mental health than some of the variables we chose do (age, sex, has children, etc). Choosing stronger predictors would likely have improved the accuracy of our model. Another weakness in our work was the fact that the dataset excluded youths younger than 15. This likely has a significant effect on our results and the predictive ability of the explanatory variable age because youth mental illness is a prevalent issue. Lastly, not all outcomes in the dataset were equally represented. For example, there were many fewer responses of "Separated" compared to more common marital statuses. Therefore, the model may have overfit to values with smaller sample sizes, impacting the generalizability of the model.

In the future, to predict mental health, we can move away from predicting subjective self-rating values and instead analyze objective behaviors such as number of therapist visits or data of a similar kind. We could also search for and add different explanatory variables that could better predict mental health than some of the ones we chose. Lastly, we hope to prevent overfitting when using a multinomial logistic regression model by making sure each value has a sufficient sample size. When changing the data source, we will make sure that all values in the population that we are interested in are properly represented to prevent the problem we encountered with the 2017 GSS dataset with missing information about the mental health of individuals less than 15 years of age.
...
# References
  ...
  Government of Canada, S. C. (2020, April 30). General Social Survey – Family (GSS). Government of Canada, Statistics Canada. https://www.statcan.gc.ca/eng/survey/household/4501. 
  
  Government of Canada, S. C.s (2019). General Social Survey - Family (GSS). https://www23.statcan.gc.ca/imdb/p2SV.pl?Function=getSurvey. 
  
  Government of Canada, S. C.s (2016). General Social Survey C31 MAIN SURVEY- FAMILY. https://www23.statcan.gc.ca/imdb/p3Instr.pl?Function=assembleInstr&lang=en&Item_Id=335815#qb345205
  
  Government of Canada, S. C.s (2016). Age and Sex Highlight Tables, 2016 Census. https://www12.statcan.gc.ca/census-recensement/2016/dp-pd/hlt-fst/as/Table.cfm?Lang=E&T=21
  ...
  Ng, A., Jordan, M. (2020). On Discriminative vs. Generative Classifiers: A comparison of logistic regression and naive Bayes. Advances in Neural Information Processing Systems 14. http://papers.nips.cc/paper/2020-on-discriminative-vs-generative-classifiers-a-comparison-of-logistic-regression-and-naive-bayes.pdf
